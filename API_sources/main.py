...
# NOTE: 動作確認用にダミークラスを定義します。実際には上の行を使用してください。
class MT5ConnectionError(Exception): pass
class TradeExecutionError(Exception): pass
class MT5Handler:
    def __init__(self, login, password, server, path): logging.info(f"MT5 Handler Initialized for login {login}")
    def connect(self): logging.info("MT5 connect called."); return True
    def shutdown(self): logging.info("MT5 shutdown called.")
    def get_terminal_info(self): return {"status": "mock_ok", "connected": True}
    def get_account_info(self): return {"login": 12345, "balance": 10000.0, "currency": "USD"}
    def get_tradable_symbols(self): return ["EURUSD", "USDJPY"]
    def get_all_symbol_details(self): return [{"name": "EURUSD"}, {"name": "USDJPY"}]
    def get_symbol_info(self, symbol): return {"name": symbol, "ask": 1.1, "bid": 1.099} if symbol == "EURUSD" else None
    def get_historical_data(self, **kwargs): return [{"time": datetime.now().isoformat(), "open": 1.1, "high": 1.101, "low": 1.099, "close": 1.1}]
    def place_order(self, request): return {"retcode": 10009, "deal": 123, "order": 456, "comment": "Order executed"}
    def get_open_positions(self, symbol=None, ticket=None):
        if ticket: return self.get_position_by_ticket(ticket)
        return [{"ticket": 1, "symbol": "EURUSD", "type": "BUY", "volume": 0.1}]
    def get_position_by_ticket(self, ticket): return [{"ticket": ticket, "symbol": "EURUSD", "type": "BUY", "volume": 0.1}] if ticket == 1 else []
    def close_position(self, **kwargs): return {"retcode": 10009, "comment": "Position closed"}
    def modify_position_sltp(self, **kwargs): return {"retcode": 10009, "comment": "Position modified"}
    def get_pending_orders(self, symbol=None, ticket=None):
        if ticket: return self.get_order_by_ticket(ticket)
        return []
    def get_order_by_ticket(self, ticket): return []
    def cancel_pending_order(self, ticket): return {"retcode": 10009, "comment": "Order canceled"}
...
class TradeRequest(BaseModel):
    ...
    type_time: Optional[OrderTimeType] = Field(OrderTimeType.GTC, description="注文有効期限タイプ")
    type_filling: Optional[OrderFillingType] = Field(OrderFillingType.IOC, description="注文執行条件")
...
def startup_event():
    ...
    try:
        settings = Settings()
        # NOTE: 実際には mt5_handler.py の MT5Handler クラスが使われます
        handler_instance = MT5Handler(
            login=settings.mt5_login,
            password=settings.mt5_password,
            server=settings.mt5_server,
            path=settings.mt5_path
        )
        # 実際の接続処理を呼び出す (mt5_handler.pyにconnectメソッドを実装していると仮定)
        if hasattr(handler_instance, 'connect') and handler_instance.connect():
             mt5_handler = handler_instance
             logging.info("Successfully connected to MetaTrader 5.")
        else:
             # ダミーハンドラ用のフォールバック（connectがない場合や失敗した場合）
             mt5_handler = handler_instance # ダミーではインスタンスをそのまま設定
             logging.warning("MT5 connect method not found or failed. Using handler instance directly. (Might be in mock mode)")
...
def shutdown_event():
    """アプリケーション終了時にMT5から切断"""
    if mt5_handler and hasattr(mt5_handler, 'shutdown'):
...
def place_order(request: TradeRequest, handler: MT5Handler = Depends(get_mt5_handler)):
    ...
    request_dict['action'] = request.action.value
    request_dict['type'] = request.type.value
    if request.type_time:
        request_dict['type_time'] = request.type_time.value
    if request.type_filling:
        request_dict['type_filling'] = request.type_filling.value
...
def get_position_by_ticket(ticket: int, handler: MT5Handler = Depends(get_mt5_handler)):
    position_list = handler.get_position_by_ticket(ticket=ticket)
...
def get_order_by_ticket(ticket: int, handler: MT5Handler = Depends(get_mt5_handler)):
    order_list = handler.get_order_by_ticket(ticket=ticket)
    if not order_list:
